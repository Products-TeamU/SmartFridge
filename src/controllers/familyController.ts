...
// Обновление названия семьи (только admin)
...
// Выход из семьи (для обычных участников)
export const leaveFamily = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (member.role === 'admin') {
      return res.status(403).json({ message: 'Администратор не может покинуть семью. Используйте удаление семьи.' });
    }
