...
export const useProductStore = create<ProductStore>((set, get) => ({
    ...
    // productStore.ts (фрагмент)
updateProduct: async (id, updatedData) => {
  try {
    // 1. Найдем текущий продукт, чтобы знать его ownerType
    const currentProduct = get().products.find(p => p._id === id);
    if (!currentProduct) throw new Error('Продукт не найден локально');

    // 2. Подготовим данные для отправки. Важно: нельзя отправлять ownerId или ownerType, если они не меняются.
    // Создаем объект только с теми полями, которые разрешено менять.
    const payload: any = {};
    ...
    // !!! Не отправляем ownerId и ownerType при обновлении !!!
    ...
    // Пробросим ошибку дальше, чтобы компонент мог ее обработать
    throw err;
  }
},
